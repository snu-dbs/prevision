...
def gen_data_sparse(rows, cols, density, name, path=None, cxn=None, write_to_disk=True):
    ...
    if cxn is None:
        cxn = sql_cxn.SQLCxn(username='postgres', db='postgres', port=15432, host='0.0.0.0')

    if cxn.table_exists(name):
        print 'WARNING: Table exists. Delete it to re-create'
        return 0
...
def write_sparse_meta(name, path, cxn=None):
    if cxn is None:
        cxn = sql_cxn.SQLCxn(username='postgres', db='postgres', port=15432, host='0.0.0.0')
