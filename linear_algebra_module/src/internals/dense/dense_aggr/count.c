                                             ...
                                             uint64_t iteration)
{
    ...
    for (uint32_t d = 0; d < opnd->dim_len; d++)
    {
        ...
        else // Selected
        {
            ...
            // Group By Dim Assertions
            assert((opnd->chunk_domains[d][1] - opnd->chunk_domains[d][0]) ==
                   (result->chunk_domains[selected_idx][1] - result->chunk_domains[selected_idx][0]));
            selected_idx++;
        }
    }
    ...
    /* Iterate each partition to generate/update the state */
    // Int
    if (opnd_attr_type == TILESTORE_INT32)
    {
        ...
        opnd_cell_value = chunk_iterator_get_cell_int(opnd_iter);
        partition_count++;
        ((int *)state[0])[num_cell_in_result_chunk - 1] += partition_count;
    }
