                                          ...
                                          uint64_t iteration)
{
    ...
    if (opnd_attr_type == TILESTORE_INT32)
    {
        ...
        for (uint64_t i = 0; i < num_cell_in_opnd_chunk; i++)
        {
            int original_max = ((int *)state[0])[((uint64_t *)state[3])[i]];
            ((int *)state[0])[((uint64_t *)state[3])[i]] =
                opnd_buf[i] > original_max
                    ? opnd_buf[i]
                    : original_max;
        }
    }
    else if (opnd_attr_type == TILESTORE_FLOAT32)
    {
        ...
        for (uint64_t i = 0; i < num_cell_in_opnd_chunk; i++)
        {
            float original_max = ((float *)state[0])[((uint64_t *)state[3])[i]];
    ...
    else if (opnd_attr_type == TILESTORE_FLOAT64)
    {
        ...
        for (uint64_t i = 0; i < num_cell_in_opnd_chunk; i++)
        {
            double original_max = ((double *)state[0])[((uint64_t *)state[3])[i]];
                                           ...
                                           uint64_t iteration)
{
    ...
    if (opnd_attr_type == TILESTORE_INT32)
    {
        ...
        {
            partition_max = (iteration == 0 ? INT_MIN : ((int *)state[0])[i]);
        ...
        partition_max = (iteration == 0 ? INT_MIN : ((int *)state[0])[num_cell_in_result_chunk - 1]);
