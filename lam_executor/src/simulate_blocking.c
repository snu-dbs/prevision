...
void _fill_future(Array* array, uint64_t idx, uint64_t *ts) {
    ...
    uint64_t RES_EOCHUNK = get_num_of_chunks(array);
    ...
    if (array->array_num == 0) {
        ...
        switch (array->op_type) {
            ...
            case OP_MAP: {
                for (uint64_t p = 0; p < RES_EOCHUNK; p++) {
                    _fill_future(array->arraylist[0], p, ts);
                    insert_or_update_future_hash(array->arraylist[0], p, ts);
                    insert_or_update_future_hash(array, p, ts);

                    array->simulated_materialized_list[p] = true;
                }
        ...
        switch (array->op_type) {
            ...
            case OP_DIV: {
                for (uint64_t p = 0; p < RES_EOCHUNK; p++) {
                    _fill_future(array->arraylist[0], p, ts);
                    _fill_future(array->arraylist[1], p, ts);
                    insert_or_update_future_hash(array->arraylist[0], p, ts);
                    insert_or_update_future_hash(array->arraylist[1], p, ts);
                    insert_or_update_future_hash(array, p, ts);
            ...
            case OP_MATMUL: {
                ...
                for (uint64_t p = 0; p < RES_EOCHUNK; p++) {
                    ...
                    calculate_chunk_coord(array->desc.dim_len, p, num_chunk_per_dim, result_chunk_coord);
    ...
    switch (array->op_type) {
        case OP_AGGR: {
            ...
            for (uint64_t i = 0; i < partition_loop; i++) {
                ...
                // array->simulated_materialized_list[i] = true;
            }
        }
    }
...
void fill_future(Array* array) {
    ...
    for (int d = 0; d < array->desc.dim_len; d++)
        EOCHUNK *= ceil((array->desc.dim_domains[d][1] - array->desc.dim_domains[d][0] + 1) / array->desc.tile_extents[d]);
