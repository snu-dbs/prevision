    ...
    else
    {
        ...
        if (res_array->array_num == 0)
        {
            ...
            case OP_SCAN:
                ...
                break;
            case OP_NEW_ARRAY:
                return_chunk = lam_operation_newarray(res_array, res_pos);
                break;
            default:
                assert(0);
                break;
            }
        } else if (res_array->array_num == 1) {
            ...
            case OP_EXPONENTIAL:
            {
                ...
                chunk_list[0] = get_pos(res_array->arraylist[0], res_pos, chunksize_child[0]);
                return_chunk = lam_operation(chunk_list, res_array->arraylist, res_array, res_pos, chunksize_parent);
            ...
            case OP_TRANSPOSE:
            {
                ...
                bf_util_calculate_nd_from_1d_row_major(res_pos, parent_chunk_lens, dim_len, &parent_coords);
                ...
                // printf("%lu,%lu -> %lu,%lu\n", res_pos[0], res_pos[1], child_pos[0], child_pos[1]);
            ...
            case OP_AGGR:
            {
                ...
                return_chunk = lam_operation_aggr(res_array, res_pos, chunksize_parent, chunksize_child[0], dim_selection_map, partition_loop);
            ...
            case OP_INV:
            {
                ...
                for (uint64_t k = 0; k < EOCHUNK; k++)
                {
                    ...
                    Chunk *decomp_chunk = scan_without_range(res_array, pos,
                                                             chunksize_child[0]);
                    chunk_getbuf(opnd_chunk, BF_EMPTYTILE_NONE);
                    chunk_getbuf(decomp_chunk, BF_EMPTYTILE_SPARSE_CSR);
            ...
            case OP_DIV:
            {
                ...
                chunk_list[0] = get_pos(res_array->arraylist[0], res_pos, chunksize_child[0]);
                chunk_list[1] = get_pos(res_array->arraylist[1], res_pos, chunksize_child[1]);
                return_chunk = lam_operation(chunk_list, res_array->arraylist, res_array, res_pos, chunksize_parent);
            ...
            case OP_MATMUL:
            {
                ...
                if (blocking_matmul) {
                    ...
                    // return_chunk = lam_operation(chunk_list, res_array->arraylist, res_array, res_pos, chunksize_parent);
                    return_chunk = lam_operation_matmul(res_array->arraylist, res_array, res_pos, chunksize_parent, chunksize_child);
